"""The load step against a real SQLAlchemy engine, schema included.

A SQLite file is a real database: it applies the primary keys, it raises
``IntegrityError`` on a duplicate, and it is what the pipeline writes to by default.
Nothing here is a double — that is the whole point, since the property being checked is
that the **database**, and not the loader, refuses a second copy of a row.
"""
